var searchFields = function(searchString, limit) {
	...
	var res, results = ItemsCollection.find(App.search.buildCondition('searchableWord', words), opts).fetch();
	// console.log('Searching in words ', opts.limit, 'found', results.length);
	opts.limit = limit - results.length;
	if (results.length < limit) {
		res = ItemsCollection.find(App.search.buildCondition('searchablePhrase', words), opts).fetch();
		// console.log('Searching in phrase ', opts.limit, 'found', res.length);
		results = results.concat(res);
		if (results.length < limit) {
			opts.limit = limit - results.length;
			res = ItemsCollection.find(App.search.buildCondition('searchableDescription', words), opts).fetch();
			// console.log('Searching in description ', opts.limit, 'found', res.length);
			results = results.concat(res);
			if (results.length < limit) {
				opts.limit = limit - results.length;
				res = ItemsCollection.find(App.search.buildCondition('searchableExample', words), opts).fetch();
